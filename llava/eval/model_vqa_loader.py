...
def eval_model(args):
    ...
    for ib, ((input_ids, image_tensor, image_sizes), line) in enumerate(tqdm(zip(data_loader, questions), total=len(questions), disable=(args.rank != 0))):
        ...
        this_answer = {"ib": ib,
                       ...
                       "model_id": model_name,
                       "metadata": {}}
        answer_list.append(this_answer)
    del model
    torch.cuda.empty_cache()
    answer_gather_list = [None for _ in range(args.world_size)] if args.rank == 0 else None
    torch.distributed.gather_object(answer_list, answer_gather_list)
    if args.rank == 0:
        for l in answer_gather_list[1:]:
            answer_gather_list[0].extend(l)
        answer_gather_list = answer_gather_list[0]
        ans_file = open(answers_file, "w")
        for answer in answer_gather_list:
