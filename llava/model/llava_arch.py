...
from llava.mm_utils import (
    crop_images_to_subimages,
    combine_subimages_to_images,
)
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            if hasattr(self.model.config, "extra") and "zoom_ratio" in self.model.config.extra:
                ...
                with torch.no_grad():
                    if images.shape[-1] <= 336:
                        ...
                        hd_features = hd_features.flatten(2).permute(0, 2, 1).half()
            if hasattr(self.model.config, "extra") and "lowres_size" in self.model.config.extra:
                ...
                lowres_size = 336
            images = torch.nn.functional.interpolate(images, size=lowres_size, mode="bilinear", antialias=True)
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                length += len(cur_input_embeds_no_im[i])
                if i < num_images:
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features)
                    cur_new_labels.append(torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=cur_labels.device, dtype=cur_labels.dtype))
                    image_range_list[-1].append((length, length + len(cur_image_features)))
                    length += len(cur_image_features)
        ...
        if new_labels is not None:
            for i in range(len(new_labels)):
                if len(image_range_list[i]) == 0:
                    continue
                ids = (new_labels[i] != -100).nonzero().reshape(-1)
                start_ids = ((torch.roll(ids,1) - ids) != -1).nonzero().reshape(-1).tolist()
                start_ids += [len(ids)]
